...
**Build a cohesive infographic-style visualization that includes*at least three*different, but complementary visualizations that work together to tell a complete story.**
...
plot_ly(data = sunburstDF,
        ...
        type = 'sunburst',
        branchvalues = 'total',
        textinfo = 'label+text',  # only include label
        hoverinfo = 'label+text',  # show label and text on hover
        text = text_selected,  # set initial text to display EV charging ports for top 10 states
...
# "#FCF7ED" "#E5EFD8" "#CEE8C3" "#B7E0AF" "#A1D99B"
```
...
Traffic proximity and volume $≥$ 90th percentile AND are at or above the 65th percentile for low income.
...
Dumbell plot for selected states only
...
ggplot(ev_dac_us_chart) +
  ...
  #                y = state_name, color = "% as of state population"),
  #            size = 2.5) +
  geom_emoji(aes(x = ev_ports_per_state_pop_pct,
                 y = state_name, emoji = "% as of state population"),
             emoji = "1f697") +
  geom_emoji(aes(x = total_ports_dac_pop_pct,
                 y = state_name, emoji = "% as of population of DACs in proximity to traffic"),
             emoji = "1f698") +
  # geom_point(aes(x = total_ports_dac_pop_pct,
  #                y = state_name, color = "% as of population of DACs in proximity to traffic"),
  #            size = 2.5) +
  ...
  theme(legend.position = "top",
        ...
        plot.background = element_rect("#FCF7ED"),
        panel.background = element_rect("#FCF7ED"),
        legend.background = element_rect("#FCF7ED"))

```

### Question 3.

Where are the disadvantaged communities suitable for EV charging deployment within California?
